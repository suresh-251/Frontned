...
function MiddlePanel({ lead }) {
  ...
  const loadCommunications = useCallback(async () => {
    ...
    try {
      ...
      const mapped = data.map((c, index) => ({
        ...
        notes: c.description,
        date: c.date,
        time: new Date(c.date).toLocaleTimeString("en-US", {
          hour: "2-digit",
          minute: "2-digit"
        })
      }));
  ...
  try {
    ...
    await leadsAPI.addCommunication(payload);
        ...
        {ACTIVITY_TABS.map(({ k, icon: Ic }) => (
          ...
          <button
            ...
            style={{
              padding: "10px 16px",
              border: "none",
              borderBottom: activeTab === k ? "2px solid #4f46e5" : "none",
              background: "none",
              color: activeTab === k ? "#4f46e5" : "#6b7280",
              fontWeight: 600,
              display: "flex",
              alignItems: "center",
              gap: 6
      ...
      <div style={{ padding: 16 }}>
        ...
        {focused && (
          ...
          <div style={{ marginTop: 8, display: "flex", gap: 8 }}>
            ...
            <button
              onClick={() => { setFocused(false); setNotes("") }}
              style={{
                padding: "6px 14px",
