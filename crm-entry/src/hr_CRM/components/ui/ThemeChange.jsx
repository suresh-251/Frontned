...
export default function ThemeChange() {
  const [currentTheme, setCurrentTheme] = useState(localStorage.getItem("hr-crm-theme") || "light");

  useEffect(() => {
    document.documentElement.setAttribute("data-theme", currentTheme);
    localStorage.setItem("hr-crm-theme", currentTheme);
  }, [currentTheme]);

  return (
    <div className="p-2 max-h-[200px] overflow-y-auto custom-scrollbar">
      <p className="text-[9px] font-black text-slate-400 uppercase tracking-widest mb-2 px-2">Theme Gallery</p>
        ...
        {themes.map((t) => (
          ...
          >
            <div className="flex items-center gap-3">
              <div className="w-3.5 h-3.5 rounded-full border border-slate-200 shadow-sm" style={{ backgroundColor: t.color }} />
              {t.name}
            </div>
