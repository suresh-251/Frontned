...
export default function Todo() {
  ...
  return (
    <div className="flex h-[520px] w-full bg-[var(--bg-card)] border border-[var(--border-color)] rounded-2xl overflow-hidden shadow-sm font-sans mt-1 transition-colors duration-300">
      ...
      <div className="w-64 border-r border-[var(--border-color)] flex flex-col shrink-0 bg-[var(--bg-card)]">
        <div className="p-4 border-b border-[var(--border-color)]">
          <div className="flex items-center justify-between mb-3">
              ...
              {/* PERM CHECK: Only manager can add */}
              {isManager && (
                <button 
                  onClick={() => { resetForm(); setEditTask(null); setOpen(true); }}
                  className="p-1.5 bg-indigo-600 text-white rounded-lg hover:bg-indigo-700 transition active:scale-95 shadow-md shadow-indigo-500/20"
                >
                  <Plus size={14} />
                </button>
              )}
        ...
        <div className="flex-1 overflow-y-auto p-3 space-y-2 custom-scrollbar">
          ...
          {filteredTasks.slice(0, 5).map(t => (
            <div key={t.taskId} className="flex items-center gap-3 p-2 border-b border-[var(--border-color)]/30">
                <div className={`h-2 w-2 rounded-full shrink-0 ${t.status === 'Completed' ? 'bg-green-500' : 'bg-yellow-500'}`} />
            ...
            <div className="bg-[var(--bg-card)] border border-[var(--border-color)] rounded-xl overflow-hidden shadow-sm transition-colors">
              <table className="w-full text-left">
                ...
                <tbody className="divide-y divide-[var(--border-color)]/30">
                  {filteredTasks.length > 0 ? (
                    filteredTasks.map((t) => (
                      <tr key={t.taskId} className="hover:bg-indigo-500/[0.02] transition-colors">
                        <td className="px-4 py-3">
                          <p className="text-[10px] font-black text-[var(--text-main)] uppercase leading-none">{t.title}</p>
                          <p className="text-[8px] font-bold text-slate-500 mt-1 uppercase opacity-70">ID: {t.taskId}</p>
                        </td>
                        <td className="px-4 py-3">
                            <span className="text-[10px] font-bold text-[var(--text-main)] opacity-70">
                              {t.dueDate ? new Date(t.dueDate).toLocaleDateString() : "-"}
                            </span>
                        </td>
                        <td className="px-4 py-3">
                          <span className={`inline-flex items-center gap-1 px-2 py-0.5 rounded-md text-[9px] font-black uppercase border ${
                            ...
                            t.status === "InProgress" ? "bg-yellow-500/10 text-yellow-500 border-yellow-500/20" :
                        ...
                        <td className="px-4 py-3 text-right">
                            <div className="flex justify-end gap-2">
                              <button onClick={() => setViewTask(t)} className="p-1 text-slate-400 hover:text-indigo-500 transition-colors"><Eye size={14}/></button>
                              
                              {/* PERM CHECK: Only manager can Edit/Delete */}
                              {isManager && (
                                <>
                                  <button onClick={() => handleEdit(t)} className="p-1 text-slate-400 hover:text-emerald-500 transition-colors"><Edit3 size={14}/></button>
                                  <button onClick={() => handleDelete(t.taskId)} className="p-1 text-slate-400 hover:text-rose-500 transition-colors"><Trash2 size={14}/></button>
                                </>
                              )}
                            </div>
                        </td>
                    ...
                    <tr><td colSpan="4" className="py-20 text-center text-[9px] font-black text-slate-400 uppercase italic opacity-50">No tasks found</td></tr>
                  )}
                </tbody>
              </table>
            </div>
      ...
      {/* VIEW MODAL (Keep as is, all roles can view) */}
      {viewTask && (
        <div className="fixed inset-0 flex items-center justify-center z-[110] backdrop-blur-sm bg-slate-900/60 p-4" onClick={() => setViewTask(null)}>
          <div className="bg-[var(--bg-card)] w-full max-w-sm rounded-2xl shadow-2xl p-6 border border-[var(--border-color)] transition-colors duration-300" onClick={e => e.stopPropagation()}>
            <h3 className="text-xs font-black text-indigo-500 uppercase tracking-widest mb-4">Task Details</h3>
            <div className="space-y-3">
                <div><p className="text-[8px] font-black text-slate-500 uppercase">Title</p><p className="text-[10px] font-bold uppercase text-[var(--text-main)]">{viewTask.title}</p></div>
                <div><p className="text-[8px] font-black text-slate-500 uppercase">Description</p><p className="text-[10px] text-[var(--text-main)] opacity-80">{viewTask.description}</p></div>
                <div className="grid grid-cols-2 gap-4 pt-2">
                  <div><p className="text-[8px] font-black text-slate-500 uppercase">Status</p><p className="text-[10px] font-bold text-indigo-500 uppercase">{viewTask.status}</p></div>
                  <div><p className="text-[8px] font-black text-slate-500 uppercase">Assigned ID</p><p className="text-[10px] font-bold uppercase text-[var(--text-main)] opacity-80">{viewTask.assignedTo}</p></div>
                </div>
            </div>
            <button onClick={() => setViewTask(null)} className="w-full mt-6 py-2.5 bg-indigo-600 text-white text-[10px] font-black uppercase rounded-xl shadow-lg hover:bg-indigo-700 transition active:scale-95">Close</button>
          </div>
        </div>
      )}

      {/* ADD / EDIT MODAL (Guarded logic - only opens if isManager is true) */}
      {open && isManager && (
        <div className="fixed inset-0 flex items-center justify-center z-[110] backdrop-blur-sm bg-slate-900/60 p-4" onClick={() => setOpen(false)}>
          <form onSubmit={handleSubmit} className="bg-[var(--bg-card)] w-full max-w-sm rounded-2xl shadow-2xl p-6 border border-[var(--border-color)] transition-colors duration-300" onClick={e => e.stopPropagation()}>
            <h3 className="text-xs font-black text-indigo-500 uppercase tracking-widest mb-5">
              {editTask ? "Modify Task" : "New Task"}
            </h3>
            <div className="space-y-3">
              <input name="title" value={form.title} onChange={handleChange} placeholder="TASK TITLE" className="w-full text-[10px] font-bold uppercase bg-[var(--bg-body)] border border-[var(--border-color)] p-2 rounded-lg outline-none focus:ring-1 focus:ring-indigo-500/30 text-[var(--text-main)]" required />
              <textarea name="description" value={form.description} onChange={handleChange} placeholder="DESCRIPTION..." className="w-full text-[10px] bg-[var(--bg-body)] border border-[var(--border-color)] p-2 rounded-lg outline-none h-20 text-[var(--text-main)]" required />
              <div className="grid grid-cols-2 gap-3">
                <input type="number" name="assignedTo" value={form.assignedTo} onChange={handleChange} placeholder="USER ID" className="w-full text-[10px] bg-[var(--bg-body)] border border-[var(--border-color)] p-2 rounded-lg text-[var(--text-main)]" required />
              ...
              <select name="status" value={form.status} onChange={handleChange} className="w-full text-[10px] font-black uppercase bg-[var(--bg-body)] border border-[var(--border-color)] p-2 rounded-lg text-[var(--text-main)]">
                ...
                <option value="InProgress" className="bg-[var(--bg-card)]">In Progress</option>
                <option value="Completed" className="bg-[var(--bg-card)]">Completed</option>
              </select>
            </div>
            <div className="flex gap-2 mt-6">
              <button type="button" onClick={() => setOpen(false)} className="flex-1 py-2.5 text-[10px] font-black uppercase text-slate-400 hover:text-slate-600 transition-colors">Cancel</button>
              <button type="submit" className="flex-1 py-2.5 bg-indigo-600 text-white text-[10px] font-black uppercase rounded-xl shadow-lg hover:bg-indigo-700 transition active:scale-95">{editTask ? "Update" : "Save"}</button>
            </div>
          </form>
        </div>
      )}
