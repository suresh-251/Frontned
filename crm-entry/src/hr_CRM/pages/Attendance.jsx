    ...
    <div className="flex h-[550px] w-full bg-[var(--bg-card)] border border-[var(--border-color)] rounded-2xl overflow-hidden shadow-sm font-sans transition-colors duration-300">
      ...
      <div className="w-64 border-r border-[var(--border-color)] flex flex-col shrink-0 bg-[var(--bg-card)]">
        ...
        <div className="flex-1 overflow-y-auto p-3 space-y-4 bg-[var(--bg-body)]/30 custom-scrollbar">
          <div className="grid grid-cols-2 gap-2">
            <div className="p-3 bg-[var(--bg-card)] rounded-xl border border-[var(--border-color)] shadow-sm">
              <p className="text-[8px] font-black text-slate-400 uppercase">Active</p>
              <p className="text-lg font-black text-indigo-500">{Object.values(attendanceMap).filter(v => v.isCheckedIn).length}</p>
            </div>
            <div className="p-3 bg-[var(--bg-card)] rounded-xl border border-[var(--border-color)] shadow-sm">
              <p className="text-[8px] font-black text-slate-400 uppercase">Staff</p>
          ...
          <div>
            ...
            <div className="space-y-1.5">
              {recentActivity.length > 0 ? recentActivity.map(emp => {
                const st = attendanceMap[emp.userId];
                return (
                  <div key={emp.userId} className="flex items-center justify-between p-2 bg-[var(--bg-card)] rounded-lg border border-[var(--border-color)] shadow-sm transition-colors">
                     <div className="flex items-center gap-2 overflow-hidden">
                        <div className={`h-1.5 w-1.5 rounded-full shrink-0 ${st?.isCheckedIn ? 'bg-emerald-500 shadow-[0_0_5px_rgba(16,185,129,0.5)]' : 'bg-rose-400'}`} />
                        <p className="text-[9px] font-bold text-[var(--text-main)] truncate uppercase opacity-80">{emp.name || emp.username}</p>
                     </div>
                     <span className="text-[8px] font-black text-indigo-500 bg-indigo-500/10 px-1 rounded uppercase">{st?.lastAction.split(' ')[0]}</span>
                  </div>
                );
              }) : (
                <p className="text-[9px] text-center text-slate-400 py-4 italic font-bold uppercase">No activity today</p>
              )}
            </div>
          </div>
        </div>
      ...
      <div className="flex-1 flex flex-col min-w-0 bg-[var(--bg-card)]">
        <div className="flex items-center justify-between px-6 py-4 border-b border-[var(--border-color)] shrink-0">
          <div className="flex items-center gap-3">
            <div className="p-2 bg-indigo-600 rounded-lg text-white shadow-lg shadow-indigo-500/20"><Activity size={16} /></div>
            <div>
              <h2 className="text-xs font-black text-[var(--text-main)] uppercase tracking-tight">Attendance Log</h2>
              <p className="text-[9px] font-bold text-slate-400 uppercase">{currentUser.role} Access</p>
            </div>
          </div>
          <button onClick={loadData} className="p-2 hover:bg-[var(--bg-body)] rounded-full transition-colors group">
            <Loader2 size={16} className={loading ? "animate-spin text-indigo-500" : "text-slate-400 group-hover:text-indigo-500"} />
          </button>
        </div>
            ...
            <table className="w-full text-left border-collapse">
              <thead className="bg-[var(--bg-body)] border-b border-[var(--border-color)]">
                <tr>
                  ...
                  <th className="px-4 py-2.5 text-[9px] font-black text-slate-400 uppercase text-center tracking-widest">Logs</th>
                  <th className="px-4 py-2.5 text-[9px] font-black text-slate-400 uppercase text-center tracking-widest">Status</th>
                  <th className="px-4 py-2.5 text-[9px] font-black text-slate-400 uppercase text-right tracking-widest">Action</th>
                </tr>
              </thead>
                ...
                {filteredEmployees.map((emp) => {
                  ...
                  return (
                    <tr key={emp.userId} className="group hover:bg-indigo-500/[0.02] transition-colors">
                      <td className="px-4 py-3">
                        <div className="flex items-center gap-3">
                          <div className={`h-8 w-8 rounded-lg flex items-center justify-center text-[10px] font-black transition-colors ${st.isCheckedIn ? 'bg-indigo-600 text-white shadow-md' : 'bg-[var(--bg-body)] text-slate-400 border border-[var(--border-color)]'}`}>
                            {emp.username?.charAt(0).toUpperCase() || "U"}
                          </div>
                          <div>
                            <p className="text-[10px] font-black text-[var(--text-main)] uppercase leading-none">{emp.name || emp.username}</p>
                            <p className="text-[8px] font-bold text-slate-400 mt-1 uppercase opacity-70">{st.shiftName}</p>
                          </div>
                        </div>
                      </td>
                      <td className="px-4 py-3 text-center">
                        <span className="text-[10px] font-black text-[var(--text-main)]">{st.count}</span>
                      </td>
                      <td className="px-4 py-3 text-center">
                        <span className={`px-2 py-0.5 rounded-md text-[9px] font-black uppercase border ${
                          st.isCheckedIn ? "bg-emerald-500/10 text-emerald-500 border-emerald-500/20" : "bg-[var(--bg-body)] text-slate-400 border-transparent"
                        }`}>
                          {st.lastAction}
                        </span>
                      </td>
                      <td className="px-4 py-3 text-right">
                         <div className="flex justify-end gap-2">
                           <button disabled={st.isCheckedIn} onClick={() => handlePunch(emp.userId, "in")} className={`px-3 py-1.5 rounded-lg text-[9px] font-black uppercase transition-all ${!st.isCheckedIn ? "bg-indigo-600 text-white shadow-lg shadow-indigo-500/20 active:scale-95" : "opacity-10 cursor-not-allowed"}`}>In</button>
                           <button disabled={!st.isCheckedIn} onClick={() => handlePunch(emp.userId, "out")} className={`px-3 py-1.5 rounded-lg text-[9px] font-black uppercase transition-all ${st.isCheckedIn ? "bg-rose-500 text-white shadow-lg shadow-rose-500/20 active:scale-95" : "opacity-10 cursor-not-allowed"}`}>Out</button>
                           <button onClick={() => { setSelectedUser(emp); setShowModal(true); getAttendanceHistory(emp.userId).then(r => setHistoryData(r?.data || [])) }} className="p-1.5 text-slate-400 hover:text-indigo-500 transition-colors"><History size={14}/></button>
                         </div>
                      </td>
                    </tr>
                  );
        ...
        {showModal && (
          <div className="fixed inset-0 flex items-center justify-center z-[110] backdrop-blur-sm bg-slate-900/60 p-4" onClick={() => setShowModal(false)}>
            <motion.div 
              ...
              className="bg-[var(--bg-card)] w-full max-w-sm rounded-2xl shadow-2xl p-6 border border-[var(--border-color)] transition-colors duration-300"
              onClick={e => e.stopPropagation()}
            >
              <div className="flex justify-between items-center mb-5 pb-2 border-b border-[var(--border-color)]/30">
                  <div>
                    <h3 className="text-xs font-black text-[var(--text-main)] uppercase tracking-widest">Employee Log</h3>
                    <p className="text-[9px] font-bold text-indigo-500 uppercase">{selectedUser?.name || selectedUser?.username}</p>
                  </div>
                  <button onClick={() => setShowModal(false)} className="bg-[var(--bg-body)] p-1 rounded-lg text-slate-400 hover:text-rose-500 transition-all"><X size={16}/></button>
              </div>
              <div className="space-y-2 max-h-60 overflow-y-auto pr-1 custom-scrollbar">
                {historyData && historyData.length > 0 ? historyData.map((log, i) => (
                  <div key={i} className="p-3 bg-[var(--bg-body)] rounded-xl border border-[var(--border-color)]/50 flex items-center justify-between group hover:border-indigo-500/30 transition-all">
                     <div>
                        <p className="text-[8px] font-black text-slate-400 uppercase tracking-widest">{log.attendanceDate?.split('T')[0]}</p>
                        <p className="text-[10px] font-bold text-[var(--text-main)] uppercase">{log.checkInTime?.slice(11, 16)} — {log.checkOutTime?.slice(11, 16) || 'ACTIVE'}</p>
                     </div>
                     <ArrowUpRight size={12} className="text-slate-400 group-hover:text-indigo-500" />
                  </div>
                )) : (
                  <p className="text-[10px] text-center text-slate-400 py-4 uppercase font-bold italic tracking-widest">No Records Found</p>
                )}
              </div>
            </motion.div>
          </div>
