    ...
    <div className="max-w-7xl mx-auto space-y-4 p-2 font-sans text-[var(--text-main)] transition-colors duration-300">
      ...
      <div className="flex items-center justify-between px-1">
        ...
        <div className="flex items-center gap-2">
          <select className="text-[10px] font-bold bg-[var(--bg-card)] border border-[var(--border-color)] p-2 rounded-lg outline-none" value={selectedBranchId} onChange={(e) => setSelectedBranchId(e.target.value)}>
            <option value="">All Branches</option>
            {branches.map(b => <option key={b.branchId} value={b.branchId}>{b.branchName} ({b.location})</option>)}
          </select>
          {canCreate && (
            <button onClick={() => { resetForm(); setCreateOpen(true); }} className="bg-indigo-600 text-white px-4 py-2 rounded-lg text-[10px] font-black uppercase flex items-center gap-2 active:scale-95 transition-all">
                ...
                <tr key={b.id} className="hover:bg-indigo-500/[0.02]">
                  ...
                  <td className="px-5 py-4 text-right">
                    <div className="flex justify-end gap-2">
                      ...
                      {canDelete && <button onClick={() => handleDelete(b.id)} className="p-1.5 text-slate-400 hover:text-rose-600 transition-all"><Trash2 size={14}/></button>}
                    </div>
                  </td>
                </tr>
              );
          ...
          <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm" onClick={() => setCreateOpen(false)}>
             <motion.div initial={{ y: 15, opacity: 0 }} animate={{ y: 0, opacity: 1 }} exit={{ y: 15, opacity: 0 }} className="bg-[var(--bg-card)] w-full max-w-[380px] rounded-2xl shadow-2xl overflow-hidden border border-[var(--border-color)]" onClick={e => e.stopPropagation()}>
                ...
                <form onSubmit={handleSubmit} className="p-5 space-y-4">
                   ...
                   <button type="submit" disabled={submitting} className="w-full py-2.5 bg-indigo-600 text-white text-[10px] font-black uppercase rounded-xl hover:bg-indigo-700 active:scale-95 transition-all shadow-md">Confirm Allocation</button>
                </form>
             </motion.div>
          </div>
        )}
