...
import {
  History, Search, Loader2, ChevronRight, LayoutDashboard, Lock
} from "lucide-react";
...
export default function OvertimeRecord() {
  ...
  useEffect(() => {
    ...
    if (isManager) {
      // Manager: load personnel list for sidebar
      ...
      // HR_USER: auto-load own records
      const loadOwnRecords = async () => {
        setDetailsLoading(true);
        try {
          const [recData, weeklyData] = await Promise.all([
            getUserOvertimeRecords(currentUser.userId).catch(() => null),
            getWeeklyOvertimeSummary(currentUser.userId).catch(() => null)
          ]);
          setRecords(recData || []);
  ...
  return (
    <div className="flex h-[520px] w-full bg-[var(--bg-card)] border border-[var(--border-color)] rounded-2xl overflow-hidden shadow-sm font-sans mt-1 transition-colors duration-300">
      ...
      {/* SIDEBAR: PERSONNEL — managers only */}
      {isManager && (
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              {isManager && selectedUser && (
                <p className="text-[9px] font-bold text-slate-500 uppercase tracking-widest leading-none mt-1.5 opacity-70">
                  Reviewing: {selectedUser.name || selectedUser.username}
                </p>
              )}
              {!isManager && (
                <p className="text-[9px] font-bold text-slate-500 uppercase tracking-widest leading-none mt-1.5 opacity-70">My Records</p>
              )}
            </div>
          </div>
              ...
              <table className="w-full text-left">
                ...
                <tbody className="divide-y divide-[var(--border-color)]/30">
                  {records && records.length > 0 ? (
                    ...
                    <tr>
                      <td colSpan="3" className="py-20 text-center text-[9px] font-black text-slate-400 uppercase tracking-widest italic opacity-50">
