...
export default function Shift() {
  ...
  // ✅ PERMISSION LOGIC
  const canView = hasPermission("SHIFT_VIEW");
  const canCreate = hasPermission("SHIFT_CREATE");
  const canAssign = hasPermission("SHIFT_ASSIGN");
  const canSeeStaff = hasPermission("USER_VIEW_ALL") || hasPermission("USER_VIEW_TEAM");
  ...
  const loadAllData = async () => {
    ...
    try {
      const [sData, aData, dData, uData] = await Promise.all([
        getShifts(), 
        canSeeStaff ? getAssignedUsers() : Promise.resolve([]), 
        getDepartments(), 
        canSeeStaff ? getAdminUsers({ page: 1, pageSize: 200 }) : Promise.resolve({ users: [] })
      ]);
      setShifts(sData || []);
  ...
  return (
    <div className="max-w-7xl mx-auto space-y-3 p-1 font-sans text-[var(--text-main)] transition-colors duration-300">
      ...
      <div className="grid grid-cols-12 gap-3">
        {/* LEFT COLUMN - EXACT 7 COLUMN DESIGN */}
        ...
        {/* RIGHT COLUMN - EXACT 5 COLUMN DESIGN */}
